...
export function MessageBubble({ message }: MessageBubbleProps) {
  ...
  const renderText = (text: string) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith("<i>") && part.endsWith("</i>")) {
        return (
          <i key={i} className="text-sage-500 italic">
            {part.slice(3, -4)}
          </i>
        );
      }
  ...
  return (
    <div
      className={`flex gap-2 items-end max-w-[400px] ${!isBot ? "flex-row-reverse ml-auto mr u" : "mr"}`}
    >
      {/* Avatar */}
      <div
        className={`w-[26px] h-[26px] rounded-full flex items-center justify-center text-[10px] font-medium shrink-0 mav ${
      ...
      {/* Bubble */}
      <div
        className={`px-[13px] py-[10px] text-[13px] leading-[1.6] bub whitespace-pre-wrap ${
          isBot
